...
fn main() {
    ...
    read_csv4("sudoku.csv").unwrap();
    // dfs_3_mil("sudoku-3m.csv").unwrap();
}
...
fn read_csv4<P: AsRef<Path>>(filename: P) -> Result<(), Box<dyn Error>> {
    let file = File::open(filename)?;
    let mut rdr = csv::Reader::from_reader(file);
    ...
    for result in rdr.records() {
        ...
        let mut board = board_dfs::Board::new(example_board.puzzle);
...
fn dfs_3_mil<P: AsRef<Path>>(filename: P) -> Result<(), Box<dyn Error>> {
    let file = File::open(filename)?;
    let mut rdr = csv::Reader::from_reader(file);
